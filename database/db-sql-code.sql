...
ALTER TYPE public.account_type OWNER TO cse_rebuild3;
...
ADD CONSTRAINT fk_inventory FOREIGN KEY (inv_id) REFERENCES public.inventory (inv_id) MATCH SIMPLE ON UPDATE CASCADE ON DELETE CASCADE;
--Will delete reviews if the inventory item is deleted
--Create relationship between 'account' and 'reviews' tables
ALTER TABLE IF EXISTS public.reviews
ADD CONSTRAINT fk_account FOREIGN KEY (account_id) REFERENCES public.account (account_id) MATCH SIMPLE ON UPDATE CASCADE ON DELETE CASCADE;
--Means the reviews will be deleted if account is deleted